...
export const useShopifyCart = () => {
  ...
  const syncCartWithShopify = useCallback(async () => {
    ...
    try {
      ...
      console.log("Raw response from Shopify:", response); // Log the entire response
      ...
      if (response.node.lineItems.edges.length > 0) {
        ...
        console.log("Parsed cart items from Shopify:", shopifyCartItems); // Log the parsed cart items
  ...
  const handleAddToCart = async (variantId, quantity = 1) => {
    try {
      ...
      console.log("Updating Shopify checkout successful, syncing cart..."); // Add this log for debugging
