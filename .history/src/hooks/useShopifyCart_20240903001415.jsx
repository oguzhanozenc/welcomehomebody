...
export const useShopifyCart = () => {
  ...
  const getProductDetailsByVariantId = (variantId) => {
    console.log("Looking for Variant ID:", variantId); // Log the variant ID being searched
    ...
    if (foundItem) {
      console.log(`Product found for variant ID: ${variantId}`, foundItem); // Log found product
      return foundItem.product;
    } else {
      console.error(`Product not found for variant ID: ${variantId}`); // Log error if not found
      return null;
    }
  };
  ...
  const handleAddToCart = async (variantId, quantity = 1) => {
    try {
      ...
      console.log("Updating Shopify checkout with items:", updatedCartItems); // Log the items being updated
