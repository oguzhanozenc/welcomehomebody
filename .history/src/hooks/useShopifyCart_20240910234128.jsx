...
export const useShopifyCart = () => {
  ...
  // Sync Shopify checkout with Redux
  const syncCartWithShopify = useCallback(async () => {
    ...
    try {
      ...
      console.log("Shopify sync response:", response);
      ...
      if (lineItems && lineItems.length > 0) {
        ...
        dispatch(syncCartItems([])); // Clear cart if Shopify cart is empty
      }
    } catch (error) {
  ...
  // Create new Shopify checkout if none exists
  ...
  // Update Shopify checkout with items
